...
void loop()
{
  ...
  if ((WiFiMulti.run() == WL_CONNECTED)) {
    ...
    http.begin(client, "http://192.168.0.9:81/data/power.json"); //HTTP
    ...
    display.setBrightness(BRIGHT_HIGH);
    if ((power.length() > 0 ) && (keep_power != power)) {
      keep_power = power;

      int str_len = power.length();
      int brank = 6 - str_len;
      ...
      display.showString(power.c_str(), str_len, brank);
    }
    delay(TEST_DELAY);
  }
}
